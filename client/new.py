...
st.markdown("""
<div class="navbar">
    <div class="nav-left"> Clinker Optimization</div>
    <div class="nav-right">
        <span>Overview</span>
        <span>Network</span>
        <span>Inventory</span>
...
with col1:
    st.markdown("📦")
with col2:
    st.title("Clinker Allocation & Optimization")
    st.caption("MILP-Based Supply Chain Optimizer")
with col3:
...
with tab3:
    ...
    fig_inv.update_layout(
        ...
        plot_bgcolor='#ffffff',
        font=dict(color='#1a1a1a', size=13, family="Arial, sans-serif"),
        xaxis=dict(gridcolor='#90AB8B', gridwidth=0.5),
        yaxis=dict(gridcolor='#90AB8B', gridwidth=0.5)
    )
...
with tab4:
    ...
    with col1:
        ...
        col_a, col_b, col_c = st.columns(3)
        with col_a:
            demand_up = st.button("▶️ Demand +20%", use_container_width=True)
        with col_b:
            demand_down = st.button("▶️ Demand -15%", use_container_width=True)
        with col_c:
            fuel_up = st.button("▶️ Fuel Cost +15%", use_container_width=True)
        ...
        if demand_up or demand_down or fuel_up:
            scenario_name = "Demand +20%" if demand_up else ("Demand -15%" if demand_down else "Fuel Cost +15%")
            ...
            with col_x:
                st.metric("Baseline", "$4.25M", label="Current")
            with col_y:
                st.metric("Scenario Result", f"${new_cost}M", delta=delta, delta_color="inverse")
        else:
...
st.markdown("---")
st.caption("Clinker Allocation & Optimization Dashboard | MILP-Based Supply Chain Optimizer")